...
public class UserLoginController {
	...
	/**
	 * url:/user/login 参数：username password 返回值：json 请求限定的方法：post
	 */
	...
	/**
	 * url:/user/token/{token} 参数：token 返回值：json 请求限定的方法：get
	 */
	...
	public Object logout(@PathVariable String token, String callback) {
		...
		try {
			result = loginservice.logoutByToken(token);
		} catch (Exception e) {
			e.printStackTrace();
			result = TaotaoResult.build(500, "出错了");
		}
